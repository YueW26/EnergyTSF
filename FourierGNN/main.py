...
parser.add_argument('--data', type=str, default='France_processed_0', help='data set')
parser.add_argument('--embed_size', type=int, default=128, help='hidden dimensions')
parser.add_argument('--hidden_size', type=int, default=64, help='hidden dimensions') # 256 128 64
...
forecast_loss = nn.MSELoss(reduction='mean').to(device)  # Move loss function to device
...
def validate(model, vali_loader):
    ...
    for i, (x, y) in enumerate(vali_loader):
        cnt += 1
        y = y.float().to(device)  # Ensure y is on the correct device
        x = x.float().to(device)  # Ensure x is on the correct device
        forecast = model(x)
...
def test():
    ...
    model_path = os.path.join(result_test_file, f"model_epoch_{args.train_epochs - 1}.pth")
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model file not found: {model_path}")
    print(f"Loading model from: {model_path}")
    
    # Reinitialize the model with the same structure
    model = FGN(pre_length=train_set.pred_len, embed_size=args.embed_size, seq_length=train_set.seq_len, hidden_size=args.hidden_size)
...
if __name__ == '__main__':
    for epoch in range(args.train_epochs):
        ...
        model_save_path = os.path.join(result_train_file, f"model_epoch_{epoch}.pth")
