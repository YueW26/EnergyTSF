...
class TPGNN(nn.Module):
    # softlap+outerwave
    def __init__(self, d_attribute, d_out, n_route, n_his, dis_mat=None, kt=2, n_c=10, droprate=0., temperature=1.0):
        ...
        # 设置邻接矩阵
        if dis_mat is None:
            # 如果 dis_mat 为空，则使用单位矩阵
            self.distant_mat = torch.eye(n_route)
        else:
    ...
    def forward(self, x, stamp):
        ...
        for i in range(1, self.kt):
            z_fix = adj_2 @ z_fix + (x @ w_stack[i] * (temp_1[:, i].reshape(b, 1, 1, 1))).permute(0, 2, 1, 3).reshape(b * t, n, -1)
